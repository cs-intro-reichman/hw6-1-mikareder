...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		Color [][] image_flipped = new Color[image.length][image[0].length]; 
		if(image!=null){
		for(int i=0; i<image.length;i++){
			for (int j=0; j<image[0].length;j++){
				image_flipped[i][j]= image [i][image[0].length-1-j];
			}
		}
		}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for(int i=0; i<image.length;i++){
			for (int j=0; j<image[0].length;j++){
				image_flipped[i][j]= image [image.length-1-i][j];
		}
		}
	}
		return image_flipped;
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            int srcRow = (int)(i * ((double)h0 / height));
            int srcCol = (int)(j * ((double)w0 / width));
